...
<script>
	var calcularSalario = function(salario, ajuste, nuevosalario){
		...
		var ajusteSalario;
		var nuevoSal;
		var presupuestoEjecutado;
		var saldoPresupuesto;

		salarioActual = parseInt(document.getElementById(salario).value);
		ajusteSalario = parseInt(document.getElementById(ajuste).value);
		presupuestoEjecutado = parseInt(document.getElementById('txtPresupuestoPropuesto').value);
				
		nuevoSal = parseInt(salarioActual + ((salarioActual * ajusteSalario)/100));
		document.getElementById(nuevosalario).value = nuevoSal;
		document.getElementById('txtPresupuestoPropuesto').value = presupuestoEjecutado - 
		( parseInt(document.getElementById(salario).value) * 10/100) 
		+ (parseInt(document.getElementById(salario).value) * parseInt(document.getElementById(ajuste).value)/100);
		
		document.getElementById('txtSaldoPresupuesto').value = document.getElementById('txtPresupuestoPauta').value - document.getElementById('txtPresupuestoPropuesto').value
...
<table>
	...
	<tbody>
		<tr>
			<td>
				<input type="text" disabled id="txtPresupuestoPauta" name="txtPresupuestoPauta" value="<?=$presupuestoPais->presupuesto?>"/>		
			</td>
			<td>
				<input type="text" disabled id="txtPresupuestoPropuesto" name="txtPresupuestoPropuesto" value="<?=$presupuestoPropuesto?>"/>
			</td>
			<td>
				<input type="text" disabled id="txtSaldoPresupuesto" name="txtSaldoPresupuesto" value="<?=$presupuestoPais->presupuesto - $presupuestoPropuesto?>"/>
			</td>
			<td></td>
			<td></td>
			<td></td>
...
<table>
		...
		foreach ($nomina as $nomina_item):
			...
			echo form_open('SalaryManagementAdmin/proponerCompensacion');
			?>
			<tr>
			...
			<!-- Eval. Desempeño -->
				<td><b>Eval. Desemp. </b><br>
					<select id="evaldesempeno_<?=$nomina_item['legajo']?>" name="evaldesempeno_<?=$nomina_item['legajo']?>"
					<?php if($nomina_item['app_status']!='INICIADO' && $nomina_item['app_status']!='RECHAZADO'){?> disabled <?php } ?>>
			...
			<!-- Pos.del Rango -->
				<td><b>Rango </b><br>
					<select id="posdelrango_<?=$nomina_item['legajo']?>" name="posdelrango_<?=$nomina_item['legajo']?>"
					<?php if($nomina_item['app_status']!='INICIADO' && $nomina_item['app_status']!='RECHAZADO'){?> disabled <?php } ?>>
			...
			<!-- Prioridad ajuste -->
				<td><b>Prioridad Ajuste</b><br>	
					<select id="prioridaddeajuste_<?=$nomina_item['legajo']?>" name="prioridaddeajuste_<?=$nomina_item['legajo']?>"
					<?php if($nomina_item['app_status']!='INICIADO' && $nomina_item['app_status']!='RECHAZADO'){?> disabled <?php } ?>>
			...
			<!-- Pauta -->
				<td><b>Pauta </b><br>
					<select id="pauta_<?=$nomina_item['legajo']?>" name="pauta_<?=$nomina_item['legajo']?>"
					<?php if($nomina_item['app_status']!='INICIADO' && $nomina_item['app_status']!='RECHAZADO'){?> disabled <?php } ?>	>
			...
			<?php
				if($nomina_item['app_status']=='INICIADO' ||
				   $nomina_item['app_status']=='RECHAZADO'){
			?>
...
<div id="nuevoSalario" class="tabcontent" name="nuevoSalario">
	<table>
		...
		foreach ($nomina as $nomina_item):
			echo validation_errors();
			echo form_open('SalaryManagementAdmin/proponerPorcentajeSalario');
			?>
			<tr>
				<td><input type="text" size="8" id="txtLegajo" name="txtLegajo" value="<?php echo $nomina_item['legajo']?>"/></td>
				...
				?>
						<td><input type="text" size="8" id="txtAjusteSalario_<?=$nomina_item['legajo']?>" name="txtAjusteSalario_<?=$nomina_item['legajo']?>" value="<?=$nomina_item['app_porcentaje_propuesto']?>"/></td>
						<td><input type="button" onclick="calcularSalario('txtSalario_<?=$nomina_item['legajo']?>','txtAjusteSalario_<?=$nomina_item['legajo']?>','txtNuevoSalario_<?=$nomina_item['legajo']?>');" value="Calcular"></button></td>
						<td><input type="text" size="8" id="txtNuevoSalario_<?=$nomina_item['legajo']?>" name="txtNuevoSalario_<?=$nomina_item['legajo']?>" value="<?=$nomina_item['app_salario_propuesto']?>"/></td>		
						<td><?=$nomina_item['app_status']?></td>
						<td><input type="submit" value="Guardar"/></td>
				<?php
					}
				...
				<?php
					if($nomina_item['app_status']=='INICIADO'){
				?>
						<td><input type="text" size="8" id="txtAjusteSalario_<?=$nomina_item['legajo']?>" name="txtAjusteSalario_<?=$nomina_item['legajo']?>"/></td>				
						<td><input type="button" onclick="calcularSalario('txtSalario_<?=$nomina_item['legajo']?>','txtAjusteSalario_<?=$nomina_item['legajo']?>','txtNuevoSalario_<?=$nomina_item['legajo']?>');" value="Calcular"></button></td>
						<td><input type="text" size="8" id="txtNuevoSalario_<?=$nomina_item['legajo']?>" name="txtNuevoSalario_<?=$nomina_item['legajo']?>"/></td>	
						<td><?=$nomina_item['app_status']?></td>
						<td><input type="submit" value="Guardar"/></td>
				<?php
				}
				...
				?>					
				<?php
					if($nomina_item['app_status']=='FINALIZADO' ||
					   $nomina_item['app_status']=='NOTIFICADO'){
				?>
						<td><input type="text" disabled size="8" id="txtAjusteSalario_<?=$nomina_item['legajo']?>" name="txtAjusteSalario_<?=$nomina_item['legajo']?>" value="<?=$nomina_item['app_porcentaje_propuesto']?>"/></td>
						<td></td>
						<td><input type="text" disabled size="8" id="txtNuevoSalario_<?=$nomina_item['legajo']?>" name="txtNuevoSalario_<?=$nomina_item['legajo']?>" value="<?=$nomina_item['app_salario_propuesto']?>"/></td>		
				...
				<?php
					if($nomina_item['app_status']=='REVISAR'){
				?>
						<td><input type="text" disabled size="8" id="txtAjusteSalario_<?=$nomina_item['legajo']?>" name="txtAjusteSalario_<?=$nomina_item['legajo']?>" value="<?=$nomina_item['app_porcentaje_propuesto']?>"/></td>
						<td></td>
						<td><input type="text" disabled size="8" id="txtNuevoSalario_<?=$nomina_item['legajo']?>" name="txtNuevoSalario_<?=$nomina_item['legajo']?>" value="<?=$nomina_item['app_salario_propuesto']?>"/></td>		
...
<div id="comentarios" class="tabcontent" name="comentarios">
	...
	<table>
		...
		foreach ($nomina as $nomina_item):
			echo validation_errors();
			echo form_open('SalaryManagementAdmin/updateObservaciones');
		?>
			<tr>
				<td><input type="text" size="8" id="txtLegajo" name="txtLegajo" value="<?php echo $nomina_item['legajo'];?>"/></td>
				<td><?php echo $nomina_item['nombre_completo'];?></td>
